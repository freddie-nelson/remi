...
{
    glm::vec4 hsla(h, s, l, a);
    if (!Color::isColorValidSafe(hsla))
    {
        throw std::invalid_argument("Invalid hsla color.");
    }
...
float Rendering::Color::hueToRgb(float p, float q, float t)
{
    if (t < 0)
        t += 1;
    if (t > 1)
